...
//const REMOTO = {host:"localhost", port: 3001}
...
const continuar = (socket, st, buf)=>{
    ...
    if (Buffer.byteLength(st.body, 'utf-8') - st.length == 0){
        ...
        if (BD[hash]){
            ...
            BD[hash] = {peticion: st.peticion, method: st.method, url: st.url, protocol: st.protocol, headers: st.headers, body: st.body}
...
exports.handle = function(msg){
    ...
    if (Buffer.byteLength(body, 'utf-8') - length < 0){
        pending.set(this, {peticion, method, url, protocol, headers, body, length})
        return
    }
