...
const log = (txt, obj, hash)=>{
    console.log(txt)
    const u = uniq()
    mainW && buflog.unshift({txt, obj, hash, uniq: u})
    mainW && mainW.webContents.send('log', {txt, obj, hash, uniq: u})
}
let bufconsole = []
const consola = exports.consola = txt=>{
    console.log(txt)
    mainW && bufconsole.unshift(txt)
    mainW && mainW.webContents.send('consola', txt)
}
const close = (s, txt)=>{
    try{
...
const completeRunning = hash=>{
    ...
    completar.forEach(it=>{
        running.delete(it.s)
        log("&" + BD[it.h].peticion, BD[it.hash], it.hash)
